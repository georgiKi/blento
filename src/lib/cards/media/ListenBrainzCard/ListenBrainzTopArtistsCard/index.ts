...
import ListenBrainzTopArtistsCard from './ListenBrainzTopArtistsCard.svelte';
import { topArtists } from './artists.remote';
import type { CardDefinition } from '../../../types';

export const ListenBrainzTopArtistsCardDefinition = {
	type: 'listenbrainzTopArtists',
	contentComponent: ListenBrainzTopArtistsCard,
	...
	loadData: async (items) => {
		...
		for (const item of items) {
			...
			try {
				const data = await topArtists(username);
				if (data) allData[`listenbrainzTopArtists:${username}`] = data ?? [];
			} catch (error) {
				console.error('Failed to fetch ListenBrainz top artists:', error);
			}
		}
		return allData;
	},
	...
	minH: 2,
	canHaveLabel: true,
	name: 'ListenBrainz Top Artists',
	keywords: ['music', 'scrobble', 'artists', 'listenbrainz', 'brainz', 'top'],
	groups: ['Media'],
...
} as CardDefinition & { type: 'listenbrainzTopArtists' };
